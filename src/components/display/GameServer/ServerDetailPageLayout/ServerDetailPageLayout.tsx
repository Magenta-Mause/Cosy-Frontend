...
const ServerDetailPageLayout = (props: { gameServer: GameServerDto, children: React.ReactNode }) => {
  ...
  return <div className="flex w-full min-h-screen">
    <div id={"gameServerDetailPage:exitButton"} className={"flex h-25 items-end w-[10%]"}>
      <Link to={"/"} tabIndex={-1}>
        <FancyNavigationButton isActive={false} label={t("serverPage.back")} tabIndex={0} direction={"right"}
                               className={"group"}>
          <>
      ...
      {TABS.map(({label, icon, path}) => (
        <div className={"relative"}>
          <div>
            ...
            >
              {({isActive}) => (
                <FancyNavigationButton isActive={isActive}
                                       label={t("serverPage.navbar." + label)}>{icon}</FancyNavigationButton>
              )}
            </Link>
          </div>
        </div>
...
const FancyNavigationButton = (props: {
  ...
  >
    ...
    <div
      className={cn("group-focus:max-w-28 group-focus:" + compiledMargin + "-1 group-focus:opacity-100",
